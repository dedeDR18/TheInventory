...
class Interactor(private val repository: IAuthRepository) : Usecase {
    override fun doCreateUser(
        ...
        kelas: String,
        noHp: String
    ) = repository.createUser(email, password, nama, nim, kelas, noHp)
    ...
    override fun doSendPeminjamanForm(dataPeminjaman: Peminjaman): LiveData<Boolean> =
        repository.createPeminjaman(dataPeminjaman)
